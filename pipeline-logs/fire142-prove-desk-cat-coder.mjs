#!/usr/bin/env node
/** fire142: headless PROVE desk-cat-coder @390 + 1440 */
...
const PAGE = '/games/desk-cat-coder.html';
const results = [];
...
async function prove(vp) {
  ...
  await page.waitForSelector('#dccPlayBtn', { timeout: 30000 });
  await page.click('#dccPlayBtn');
  await page.waitForSelector('#dccFrame', { timeout: 30000 });
  const frame = page.frameLocator('#dccFrame');
  await frame.locator('#startButton').click({ timeout: 30000 });
...
if (!existsSync(join(DIST, 'games/desk-cat-coder.html'))) {
...
server.close();
writeFileSync(join(__dirname, 'fire142-prove-result.json'), JSON.stringify({ ok: allOk, results }, null, 2));
