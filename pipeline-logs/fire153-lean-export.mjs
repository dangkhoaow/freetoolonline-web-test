...
const ROUTES = ['/games/classic-pong.html','/guides/how-to-play-classic-pong.html','/guides/classic-pong-when.html','/guides/classic-pong-vs-alternatives.html'];
...
for (const rel of ['script', 'img', 'games/classic-pong']) {
...
for (const route of ROUTES) {
  ...
  const html = await renderPageDocument({
    ...
    ioVersion: DEFAULT_IO_VERSION, deploySha: 'fire153', getAlterUploaderDelayMs: () => 0,
...
const pageHtml = await readFile(path.join(distDir, 'games/classic-pong.html'), 'utf8');
const checks = {
  playBtn: /id=["']cpPlayBtn["']/.test(pageHtml),
  headerIcon: /classicpong__[a-f0-9]{8}\.svg/.test(pageHtml),
  og: /classicpong\.png/.test(pageHtml),
  touch: /classicpong-180\.png/.test(pageHtml),
};
