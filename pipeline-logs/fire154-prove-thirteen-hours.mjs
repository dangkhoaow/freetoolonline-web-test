...
const PAGE = '/games/thirteen-hours.html';
...
async function proveAt(width) {
  ...
  await page.waitForSelector('#thPlayBtn', { timeout: 15000 });
  // header enrollment smoke
  const headerOk = await page.evaluate(() => {
    const img = document.querySelector('header img, .brand img, a[href="/"] img');
    const og = document.querySelector('meta[property="og:image"]');
    return Boolean(img && og && (og.getAttribute('content') || '').includes('thirteenhours'));
  }).catch(() => false);
  await page.click('#thPlayBtn');
  await page.waitForSelector('#thFrame', { timeout: 15000 });
  const frame = page.frameLocator('#thFrame');
  await frame.locator('#root').waitFor({ timeout: 20000 });
  await frame.locator('#infinite').waitFor({ timeout: 10000 });
  await page.waitForTimeout(400);
  const shot1 = await frame.locator('#root').screenshot();
  await frame.locator('#infinite').click();
  await frame.locator('.clock').first().waitFor({ timeout: 10000 });
  await page.waitForTimeout(600);
  await frame.locator('.g-c').click({ position: { x: 50, y: 50 } }).catch(() => {});
  await page.waitForTimeout(900);
  const shot2 = await frame.locator('#root').screenshot();
  const changed = Buffer.compare(shot1, shot2) !== 0;
  const clockCount = await frame.locator('.clock').count();
  ...
  await browser.close();
  server.close();
  return { width, changed, clockCount, headerOk, b1: shot1.length, b2: shot2.length, gameErrors };
}
const r390 = await proveAt(390);
const r1440 = await proveAt(1440);
console.log(JSON.stringify({ r390, r1440 }, null, 2));
const ok = r390.changed && r1440.changed && r390.clockCount > 0 && r1440.clockCount > 0 && r390.gameErrors.length === 0 && r1440.gameErrors.length === 0;
process.exit(ok ? 0 : 2);
