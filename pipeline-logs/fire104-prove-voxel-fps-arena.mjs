...
const srv = await serve(DIST, 9890);

async function bootGameFrame(page) {
  await page.evaluate(() => { if (typeof doAfterPageRendered === 'function') doAfterPageRendered(); });
  await page.locator('#vfaPlayBtn').click({ force: true });
  await page.waitForFunction(() => {
    return [...document.querySelectorAll('iframe')].some((f) => (f.src || '').includes('voxel-fps-arena/index.html'));
  }, { timeout: 90000 });
  await page.waitForTimeout(3000);
  const gameFrame = page.frames().find((f) => f.url().includes('voxel-fps-arena/index.html'));
...
async function prove(w, h) {
  ...
  await page.goto('http://127.0.0.1:9890/games/voxel-fps-arena.html', { waitUntil: 'domcontentloaded', timeout: 90000 });
  ...
  console.log('PROVE PASS', w + 'x' + h, 'vfaPlayBtn+canvas');
  await page.close();
}
...
try {
  ...
  console.log('fire104 PROVE ALL PASS');
} finally {
  await browser.close();
  srv.close();
}
