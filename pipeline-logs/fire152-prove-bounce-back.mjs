#!/usr/bin/env node
/** fire152: PROVE bounce-back from dist/ */
...
const PAGE = '/games/bounce-back.html';
...
async function proveAt(width) {
  ...
  await page.waitForSelector('#bbPlayBtn', { timeout: 15000 });
  await page.click('#bbPlayBtn');
  await page.waitForSelector('#bbFrame', { timeout: 15000 });
  const frame = page.frameLocator('#bbFrame');
  await frame.locator('canvas#c1').waitFor({ timeout: 20000 });
  await page.waitForTimeout(800);
  const shot1 = await frame.locator('canvas#c1').screenshot();
  // WASD + click to cause a visual change
  await page.keyboard.down('KeyD');
  await page.waitForTimeout(200);
  await page.keyboard.up('KeyD');
  const box = await page.locator('#bbFrame').boundingBox();
  if (box) {
  ...
  const shot2 = await frame.locator('canvas#c1').screenshot();
