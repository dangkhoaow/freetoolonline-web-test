#!/usr/bin/env node
/** fire151: PROVE rune-keeper from dist/ */
...
const PAGE = '/games/rune-keeper.html';
...
async function proveAt(width) {
  ...
  await page.waitForSelector('#rkPlayBtn', { timeout: 15000 });
  await page.click('#rkPlayBtn');
  await page.waitForSelector('#rkFrame', { timeout: 15000 });
  const frame = page.frameLocator('#rkFrame');
  await frame.locator('canvas').waitFor({ timeout: 20000 });
  await page.waitForTimeout(600);
  const shot1 = await frame.locator('canvas').screenshot();
  const box = await page.locator('#rkFrame').boundingBox();
  if (box) {
    const x0 = box.x + box.width * 0.25;
    const y0 = box.y + box.height * 0.55;
    await page.mouse.move(x0, y0);
    await page.mouse.down();
    await page.mouse.move(x0 + 40, y0 - 60, { steps: 6 });
    await page.mouse.move(x0 + 80, y0, { steps: 6 });
