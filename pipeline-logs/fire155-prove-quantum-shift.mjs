...
const PAGE = '/games/quantum-shift.html';
...
async function proveAt(width) {
  ...
  await page.waitForSelector('#qsPlayBtn', { timeout: 15000 });
  const headerOk = await page.evaluate(() => {
    const img = document.querySelector('header img, .brand img, a[href="/"] img');
    const og = document.querySelector('meta[property="og:image"]');
    return Boolean(img && og && (og.getAttribute('content') || '').includes('quantumshift'));
  }).catch(() => false);
  await page.click('#qsPlayBtn');
  const iframeHandle = await page.waitForSelector('#qsFrame', { timeout: 15000 });
  ...
  await gameFrame.click('canvas#game', { position: { x: 60, y: 120 }, force: true });
  await page.waitForTimeout(600);
  const shot1 = await gameFrame.locator('canvas#game').screenshot();
  await gameFrame.focus('canvas#game');
  ...
  await page.waitForTimeout(400);
  const shot2 = await gameFrame.locator('canvas#game').screenshot();
  ...
  await browser.close();
  server.close();
  return { width, changed, headerOk, b1: shot1.length, b2: shot2.length, gameErrors };
}
