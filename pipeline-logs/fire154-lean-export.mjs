...
const ROUTES = ['/games/thirteen-hours.html','/guides/how-to-play-thirteen-hours.html','/guides/thirteen-hours-when.html','/guides/thirteen-hours-vs-alternatives.html'];
...
for (const rel of ['script', 'img', 'games/thirteen-hours']) {
...
for (const route of ROUTES) {
  ...
  const html = await renderPageDocument({
    ...
    ioVersion: DEFAULT_IO_VERSION, deploySha: 'fire154', getAlterUploaderDelayMs: () => 0,
...
const pageHtml = await readFile(path.join(distDir, 'games/thirteen-hours.html'), 'utf8');
const checks = {
  playBtn: /id=["']thPlayBtn["']/.test(pageHtml),
  headerIcon: /thirteenhours__[a-f0-9]{8}\.svg/.test(pageHtml),
  og: /thirteenhours\.png/.test(pageHtml),
  touch: /thirteenhours-180\.png/.test(pageHtml),
};
