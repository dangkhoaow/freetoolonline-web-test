#!/usr/bin/env node
/** fire139: Playwright PROVE herd-cats-home @390+1440 */
...
async function prove(viewport) {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/games/herd-cats-home.html`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await page.waitForSelector('#hchPlayBtn', { timeout: 30000 });
  await page.evaluate(() => { if (typeof doAfterPageRendered === 'function') doAfterPageRendered(); });
  await page.click('#hchPlayBtn');
  await page.waitForSelector('#hchFrame', { timeout: 30000 });
  const frame = page.frameLocator('#hchFrame');
  await frame.locator('#a').waitFor({ timeout: 30000 });
  await page.waitForTimeout(7000);
  const canvas = frame.locator('#a');
  const box = await canvas.boundingBox();
  const clickAt = async (rx, ry) => {
    if (box) {
      await canvas.click({ position: { x: Math.max(4, box.width * rx), y: Math.max(4, box.height * ry) } });
    } else {
      await canvas.click();
    }
  };
  ...
  const canvasCheck = await canvasNonBlack(frame, '#a');
  const dims = await frame.locator('#a').evaluate((c) => ({ w: c.width, h: c.height }));
  await browser.close();
  return {
    viewport,
    canvasCheck,
...
server.close();
const fail = results.some((r) => !r.canvasCheck.ok || r.errors.length);
