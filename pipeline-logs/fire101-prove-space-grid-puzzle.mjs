...
const srv = await serve(DIST, 9887);

async function swipeLeft(page, gameFrame) {
  const box = await gameFrame.locator('canvas').boundingBox();
  if (!box) return;
...
async function bootGameFrame(page) {
  ...
  await page.locator('#sgpPlayBtn').click({ force: true });
  await page.waitForFunction(() => {
    return [...document.querySelectorAll('iframe')].some((f) => (f.src || '').includes('space-grid-puzzle/index.html'));
  }, { timeout: 90000 });
  await page.waitForTimeout(2500);
  const gameFrame = page.frames().find((f) => f.url().includes('space-grid-puzzle/index.html'));
  if (!gameFrame) throw new Error('game iframe missing');
  await gameFrame.waitForSelector('canvas', { timeout: 30000 });
  await swipeLeft(page, gameFrame);
...
async function prove(w, h) {
  ...
  await page.goto('http://127.0.0.1:9887/games/space-grid-puzzle.html', { waitUntil: 'domcontentloaded', timeout: 90000 });
  ...
  console.log('PROVE PASS', w + 'x' + h, 'sgpPlayBtn+canvas+swipe');
  await page.close();
}
...
try {
  ...
  console.log('fire101 PROVE ALL PASS');
} finally {
  await browser.close();
  srv.close();
}
