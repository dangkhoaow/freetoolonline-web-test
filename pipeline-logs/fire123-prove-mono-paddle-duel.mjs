...
async function prove(viewport) {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/games/mono-paddle-duel.html`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await page.waitForSelector('#mpdPlayBtn', { timeout: 30000 });
  await page.evaluate(() => { if (typeof doAfterPageRendered === 'function') doAfterPageRendered(); });
  await page.click('#mpdPlayBtn');
  await page.waitForSelector('#mpdFrame', { timeout: 30000 });
  const frame = page.frameLocator('#mpdFrame');
  await frame.locator('canvas').waitFor({ timeout: 30000 });
  const canvasOk = await frame.locator('canvas').evaluate((c) => c.width === 800 && c.height === 500);
  await browser.close();
