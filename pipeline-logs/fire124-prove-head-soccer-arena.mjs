...
async function prove(viewport) {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/games/head-soccer-arena.html`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await page.waitForSelector('#hsaPlayBtn', { timeout: 30000 });
  await page.evaluate(() => { if (typeof doAfterPageRendered === 'function') doAfterPageRendered(); });
  await page.click('#hsaPlayBtn');
  await page.waitForSelector('#hsaFrame', { timeout: 30000 });
  const frame = page.frameLocator('#hsaFrame');
  await frame.locator('canvas').waitFor({ timeout: 30000 });
  const canvasOk = await frame.locator('canvas').evaluate((c) => c.width === 900 && c.height === 520);
  await browser.close();
