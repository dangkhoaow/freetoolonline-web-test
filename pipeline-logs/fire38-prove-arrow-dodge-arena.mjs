#!/usr/bin/env node
/** PROVE arrow-dodge-arena (fire38). */
...
const URL = 'http://127.0.0.1:9879/games/arrow-dodge-arena.html';
...
async function proveViewport(page, width, height) {
  ...
  await page.waitForSelector('#adaPlayBtn');
  await page.click('#adaPlayBtn');
  const frame = page.frameLocator('#adaFrame');
  await frame.locator('#startBtn').click({ force: true });
  await frame.locator('canvas#gameCanvas').waitFor({ timeout: 15000 });
...
const server = await serve(DIST, 9879);
