...
async function prove(viewport) {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/games/mono-stack-blocks.html`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await page.waitForSelector('#msbPlayBtn', { timeout: 30000 });
  await page.evaluate(() => { if (typeof doAfterPageRendered === 'function') doAfterPageRendered(); });
  await page.click('#msbPlayBtn');
  await page.waitForSelector('#msbFrame', { timeout: 30000 });
  const frame = page.frameLocator('#msbFrame');
